...
export async function initDbSchema() {
  if (!process.env.SQL_HOST || !process.env.SQL_USER) {
    console.log('Cloud SQL credentials not set, using Firebase / Supabase mode.');
    return;
  }
  ...
  try {
    ...
    try {
      ...
      console.warn('Note on Cloud SQL schema init (permission/table state):', queryErr?.message || queryErr);
    } finally {
      client.release();
    }
  } catch (connErr: any) {
    console.warn('Cloud SQL connection notice:', connErr?.message || connErr);
  }
}
